<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Playfair+Display&size=34&pause=1000&color=9B5DE5&center=true&vCenter=true&width=700&lines=Patrícia+Rocha;Analista+de+Dados;Ciência+da+Computação" />
</p>

<p align="center">
  <strong>Analista de Dados formada</strong> • Estudante de Ciência da Computação<br>
  Desenvolvimento Web • Dados • Tecnologia aplicada a decisões reais
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Data%20Analytics-DB4C77?style=flat-square"/>
  <img src="https://img.shields.io/badge/Web%20Development-6A5ACD?style=flat-square"/>
  <img src="https://img.shields.io/badge/Python%20%7C%20SQL%20%7C%20BI-8A2BE2?style=flat-square"/>
</p>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&height=3"/>

<h2>Sobre mim</h2>

<p>
Sou <strong>formada em Análise de Dados</strong> e estudante de <strong>Ciência da Computação</strong>, com foco em transformar dados em informação estratégica e desenvolver soluções tecnológicas bem estruturadas.
</p>

<p>
Atuo com <strong>análise, visualização e exploração de dados</strong>, além do desenvolvimento de aplicações web voltadas para organização, performance e tomada de decisão baseada em dados.
</p>

<p>
Possuo experiência prática com projetos em <strong>Python, SQL, Power BI, Django e Streamlit</strong>, integrando dados, lógica e visualização.
</p>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&height=3"/>

<h2>Áreas de atuação</h2>

<ul>
  <li>Análise e exploração de dados</li>
  <li>Dashboards analíticos e indicadores de negócio</li>
  <li>Desenvolvimento de aplicações web</li>
  <li>Modelagem e consultas SQL</li>
  <li>Integração entre dados, produto e decisão</li>
</ul>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&height=3"/>

<h2>Tecnologias & Ferramentas</h2>

<h3>Linguagens & Frameworks</h3>

<p>
  <img src="https://img.shields.io/badge/Python-4B0082?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Django-3A0CA3?style=for-the-badge&logo=django&logoColor=white"/>
  <img src="https://img.shields.io/badge/Streamlit-C71585?style=for-the-badge&logo=streamlit&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQL-8A2BE2?style=for-the-badge"/>
</p>

<h3>Front-end</h3>

<p>
  <img src="https://img.shields.io/badge/HTML5-DB7093?style=for-the-badge&logo=html5&logoColor=white"/>
  <img src="https://img.shields.io/badge/CSS3-BA55D3?style=for-the-badge&logo=css3&logoColor=white"/>
  <img src="https://img.shields.io/badge/JavaScript-DDA0DD?style=for-the-badge&logo=javascript&logoColor=black"/>
</p>

<h3>Dados & Visualização</h3>

<p>
  <img src="https://img.shields.io/badge/Power%20BI-F4C430?style=for-the-badge&logo=power-bi&logoColor=black"/>
  <img src="https://img.shields.io/badge/Excel-9370DB?style=for-the-badge&logo=microsoft-excel&logoColor=white"/>
</p>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&height=3"/>

<h2>Projetos</h2>

<ul>
  <li>Dashboards analíticos desenvolvidos em <strong>Power BI</strong></li>
  <li>Análises de dados utilizando <strong>Python e SQL</strong></li>
  <li>Aplicações web com <strong>Django e Streamlit</strong></li>
  <li>Projetos acadêmicos e práticos voltados à área de dados</li>
</ul>

<p>
Os repositórios demonstram desde <strong>exploração e tratamento de dados</strong> até <strong>visualização e desenvolvimento de aplicações</strong>.
</p>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&height=3"/>

<h2>Em desenvolvimento</h2>

<ul>
  <li>SQL avançado e bancos de dados relacionais</li>
  <li>APIs REST e back-end com Python</li>
  <li>Fundamentos de Engenharia e Ciência de Dados</li>
</ul>

<img src="https://capsule-render.vercel.app/api?type=rect&color=gradient&height=3"/>

<h2>Contato profissional</h2>

<p>
  <a href="https://www.linkedin.com/in/patricia-oliveira-rocha">
    <img src="https://img.shields.io/badge/LinkedIn-6A0DAD?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>

<p align="center">
  <img src="https://media.giphy.com/media/26BRv0ThflsHCqDrG/giphy.gif" width="260"/>
</p>

